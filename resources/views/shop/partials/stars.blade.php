@php
    ...
    $iconSize = isset($size) && is_numeric($size) ? (int) $size : 16;
@endphp
<span class="stars" role="img" aria-label="Rated {{ number_format($rating, 1) }} out of 5">
    @for($i = 1; $i <= 5; $i++)
        <svg class="stars__icon {{ $i <= $rounded ? 'is-on' : '' }}" viewBox="0 0 24 24" width="{{ $iconSize }}" height="{{ $iconSize }}" aria-hidden="true">
